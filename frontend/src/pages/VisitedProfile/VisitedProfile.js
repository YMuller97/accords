...
const VisitedProfile = ({userId}) => {
  ...
  useEffect(() => {
    ...
    fetch(process.env.REACT_APP_API_ROUTE+ 'users/'+ userId)
    ...
    fetch(process.env.REACT_APP_API_ROUTE+ 'users/'+ userId+'/instruments')
      .then(response => response.json())
      .then(data => {
          setInstruments(data);
      })
    ...
    fetch(process.env.REACT_APP_API_ROUTE+ 'users/'+ userId+'/genres')
      .then(response => response.json())
      .then(data => {
          setGenres(data);
      })
  ...
  const handleCreateConversation = async () => {
    try {
      ...
      if (isAdmin) {
        ...
        const checkResponse = await fetch(process.env.REACT_APP_API_ROUTE+ 'conv/check-admin', {
          method: 'POST',
          headers: {
        ...
        if (checkData.exists) {
          ...
          const response = await fetch(process.env.REACT_APP_API_ROUTE+ 'conv/admin', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`
        ...
        const checkResponse = await fetch(process.env.REACT_APP_API_ROUTE+ 'conv/check', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
        ...
        if (checkData.exists) {
          ...
          const response = await fetch(process.env.REACT_APP_API_ROUTE+ 'conv', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`
  ...
  const handleSubmitReport = async (reportData) => {
    try {
      ...
      const response = await fetch(process.env.REACT_APP_API_ROUTE+ 'reports', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
    ...
    <div className={classes["visited-profile"]}>
      <div className={classes["profile"]}>
        <div className={classes["profile-picture"]}>
          <img src={`${process.env.REACT_APP_API_ROUTE}${visitedUser.picture_user}`} alt="" />
        </div>
